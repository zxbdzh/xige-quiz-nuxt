  ...
  <div>
    ...
    <div class="grid gap-5 md:grid-cols-[200px_1fr]">
      ...
      <aside v-if="data?.kps?.length" class="hidden md:block">
        <nav
          class="sticky top-20 rounded-lg p-3"
        ...
        >
          ...
          <ol class="space-y-1 m-0 p-0 list-none">
            <li v-for="(kp, i) in data.kps" :key="i">
              <a
                :href="`#kp-${i}`"
                class="block py-1 px-2 rounded text-[13px] text-mute hover:bg-bg-elev hover:text-brand transition-colors"
              >
                {{ toCn(i + 1) }}、{{ kp.title }}
              </a>
            </li>
      ...
      <div class="card !p-6 sm:!p-7 min-w-0">
        ...
        >
          <h3 class="flex items-center gap-2.5 text-[16px] font-semibold m-0 mb-3">
            <span
              class="w-7 h-7 rounded grid place-items-center text-[13px] font-bold shrink-0"
          ...
          <ol class="pl-9 m-0 space-y-2">
            <li v-for="(p, j) in kp.points" :key="j" class="leading-[1.75]">
              {{ p }}
            </li>
          </ol>
        </section>
...
const { data, pending } = useChapter(id)
